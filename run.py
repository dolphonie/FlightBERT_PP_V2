...
from utils import load_config_from_json, print_attrs, file_print, convert2binfromlist,graycode2binary, binary2graycode
import os
import tqdm
        ...
        for i in range(batch_szie):
            ...
            for j in range(seq):
                ...
                if self.configs.use_graycode:
                    cvt_pred = convert2binfromlist(graycode2binary(pred_list[1:]))
                    # cvt_trgt = convert2binfromlist(graycode2binary(trgt_list[1:]))
                else:
